...
const AutoVideoEditingDashboard=()=> {
  ...
  return (
    <button className="w-full flex items-center justify-start gap-2 px-3 py-3 rounded-lg bg-[#2054ff]/15 ring-1 ring-[#2054ff]/40 hover:ring-[#2054ff] text-sm">
      {icon ?? <Upload className="w-4 h-4" />}<span>+ {label}</span>
    </button>
  );
}
  ...
  return (
    <div className="inline-flex items-center gap-2 px-2 py-2 rounded-lg bg-black/30 ring-1 ring-white/10">
      <div className="h-12 overflow-hidden rounded w-28 bg-black/70 ring-1 ring-white/10">
        <img src="/mnt/data/4.png" alt="clip" className="object-cover w-full h-full" />
      </div>
...
function Chip({ children }) {
  return <div className="px-2 py-1 text-xs rounded-md bg-black/40 ring-1 ring-white/10">{children}</div>;
}

function SelectLike({ value, onClick }) {
  return (
    <button onClick={onClick} className="flex items-center justify-between flex-1 px-3 rounded-md h-9 bg-black/40 ring-1 ring-white/10">
...
function MiniBtn({ label, value, onChange, readOnly }) {
  return (
    <div className="flex items-center gap-1">
      <span className="px-2 py-1 text-xs rounded bg-black/40 ring-1 ring-white/10">{label}</span>
      <input
        type="number"
        value={value}
    ...
    <div className="min-h-screen bg-[#05070d] text-white antialiased">
      <div className="px-4 py-6 mx-auto max-w-8xl md:px-6">
        ...
        <div className="flex items-center justify-between mt-10 mb-5">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight md:text-5xl">AUTO VIDEO EDITING</h1>
        ...
        <div className="grid grid-cols-12 gap-5">
          ...
          <aside className="col-span-12 md:col-span-2">
            <div className="space-y-3">
              ...
              <UploadBtn label="Upload Video" icon={<VideoIcon className="w-4 h-4" />} />
              <UploadBtn label="Upload Audio" icon={<AudioLines className="w-4 h-4" />} />
              ...
              <div className="space-y-3">
                {media.map((m) => (
                  <div key={m.id} className="flex items-center gap-3">
                    <div className="w-16 h-12 overflow-hidden bg-black rounded-md ring-1 ring-white/10">
                      <img src={m.src} alt={m.name} className="object-cover w-full h-full" />
                    </div>
                    <div className="text-xs truncate text-white/80">{m.name}</div>
                  </div>
                ))}
          ...
          <main className="col-span-12 md:col-span-7">
              ...
              <div className="p-2">
                ...
                <div className="flex items-center justify-between px-1 pt-2">
                  <div className="flex items-center gap-2">
                    <button className="icon-btn"><SkipBack className="w-4 h-4" /></button>
                    <button className="icon-btn" onClick={() => setIsPlaying((p) => !p)}>
                      {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                    </button>
                    <button className="icon-btn"><SkipForward className="w-4 h-4" /></button>
                  </div>
                  <button className="icon-btn"><Maximize2 className="w-4 h-4" /></button>
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 mt-3 text-sm text-white/90">
              <button className="tool-btn" title="Split"><Scissors className="w-4 h-4" /></button>
              <button className="tool-btn" title="Media"><ImageIcon className="w-4 h-4" /></button>
              <button className="tool-btn" title="Text"><Type className="w-4 h-4" /></button>
              <button className="tool-btn" title="Audio"><AudioLines className="w-4 h-4" /></button>
            ...
            <div className="mt-5">
              ...
              <div className="flex flex-wrap gap-3">
                {["16:9", "9:16", "1:1", "4:5", "21:9", "Custom size"].map((s) => (
                  <button key={s} className="px-4 py-2 text-sm rounded-lg bg-black/30 ring-1 ring-white/10 hover:ring-blue-400">{s}</button>
                ))}
              </div>
            </div>
          </main>
